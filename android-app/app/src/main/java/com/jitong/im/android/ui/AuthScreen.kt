...
private fun GroupPanel(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            OutlinedButton(
                onClick = { selectedSection = "owned" },
                modifier = Modifier.weight(1f),
            ) { Text("我的群") }
            OutlinedButton(
                onClick = { selectedSection = "search" },
                modifier = Modifier.weight(1f),
            ) { Text("找群") }
            OutlinedButton(
                onClick = { selectedSection = "create" },
                modifier = Modifier.weight(1f),
            ) { Text("建群") }
        }
...
private fun GroupSearchPanel(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        state.searchResults.forEach { group ->
            Card(Modifier.fillMaxWidth()) {
                Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                    group.avatarUrl?.let { avatarUrl ->
                        RemoteSearchGroupAvatar(
                            avatarUrl = avatarUrl,
                            fallback = group.name.take(2),
                            load = loadSearchAvatar,
                            size = 56.dp,
                        )
                    } ?: AvatarView(
                        bytes = null,
...
private fun GroupCreatePanel(
    ...
    val picker = rememberLauncherForActivityResult(
        ...
        val bytes = context.contentResolver.openInputStream(uri)?.use {
            readCappedBytes(it, 11 * 1024 * 1024)
        }
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Row(
            ...
            OutlinedButton(onClick = { picker.launch("image/*") }) {
                Text(if (state.avatar == null) "选择群头像" else "更换群头像")
            }
        }
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Button(
            ...
            enabled = !state.loading && state.name.isNotBlank(),
            modifier = Modifier.fillMaxWidth(),
        ) { Text("创建群") }
    }
}

private fun readCappedBytes(
