...
服务端通过 `MediaStorage` 接口把媒体保存到 Docker Compose 内部网络中的私有 MinIO。媒体用途分为 `MESSAGE_IMAGE` 和 `AVATAR`，生命周期统一为 `TEMP → BOUND → EXPIRED`：只有消息或头像事务成功后才能进入 `BOUND`；`EXPIRED` 立即禁止访问，物理文件由可重试异步任务删除，未绑定 TEMP 媒体在 24 小时后清理。
